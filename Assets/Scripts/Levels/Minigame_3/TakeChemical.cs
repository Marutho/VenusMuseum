...
public class TakeChemical : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if(collision.tag=="Chemical")
        {
            if (tag=="purple")
            {
                if (collision.GetComponent<Lab_WinCondition>().purpleType == false)
                {
            ...
            {
                if (collision.GetComponent<Lab_WinCondition>().greenType == false)
                {
